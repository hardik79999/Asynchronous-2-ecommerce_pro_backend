...
from shop.models import (
    Product, ProductImage, User, Category,
    Specification, SellerCategory, Role
)
from shop.extensions import db
from flask_jwt_extended import jwt_required, get_jwt_identity
from shop.utils.email_service import send_category_request_email_to_admin
...
async def save_file_async(file, path):
    await asyncio.to_thread(save_file, file, path)
...
@seller_bp.route('/product', methods=['POST'])
@seller_required
async def create_product(current_seller):
    ...
    # ✅ Approval check
    is_approved = SellerCategory.query.filter_by(
        seller_id=current_seller.id,
        category_id=category.id,
        is_approved=True,
        is_active=True
    ...
    if not is_approved:
        return jsonify({"error": "Category approval required"}), 403
    ...
    if image_files and image_files[0].filename != '':
        ...
        for file in image_files:
            if file and allowed_file(file.filename):
                ...
                tasks.append(save_file_async(file, path))
                saved_image_urls.append(f"/static/uploads/products/{unique_name}")

        await asyncio.gather(*tasks)  # ⚡ parallel save
    ...
    try:
        ...
        # Images
        for i, url in enumerate(saved_image_urls):
...
# ==============================================================================
# 📦 CATEGORY REQUEST (async email fix)
# ==============================================================================
async def send_email_async(**kwargs):
    await asyncio.to_thread(send_category_request_email_to_admin, **kwargs)
...
def request_category_approval(current_seller):
    ...
    if not category_uuid:
        return jsonify({"error": "category_uuid required"}), 400

    category = Category.query.filter_by(uuid=category_uuid, is_active=True).first()
    if not category:
        return jsonify({"error": "Category not found"}), 404

    existing = SellerCategory.query.filter_by(
        seller_id=current_seller.id,
        category_id=category.id
    ).first()
